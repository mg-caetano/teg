\documentclass[12pt, twoside]{report}
\usepackage[spanish]{babel}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[final]{microtype}
\usepackage[letterpaper,width=170mm,top=25mm,bottom=25mm,bindingoffset=6mm]{geometry}
\usepackage{graphicx}
\graphicspath{ {images/} }
\usepackage[table]{xcolor}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{hyperref}
\usepackage[backend=biber,safeinputenc,sorting=none,backref=true]{biblatex}
\addbibresource{references.bib}
\usepackage{csquotes}
\usepackage{array}
\usepackage{longtable}
\renewcommand{\listtablename}{Índice de tablas}
\newcolumntype{P}[1]{>{\raggedright\arraybackslash}p{#1}}
\usepackage{multirow}
\usepackage{afterpage}
\usepackage{pdflscape}
\usepackage[page, titletoc]{appendix}
\renewcommand{\appendixname}{Apéndice}
\renewcommand{\appendixtocname}{Apéndice}
\renewcommand{\appendixpagename}{Apéndice}

\usepackage{etoolbox} % utf8 encoding support for appendix
\makeatletter
\appto{\appendices}{\def\Hy@chapapp{Appendix}}
\makeatother

\usepackage{listings}
\renewcommand{\lstlistingname}{Algoritmo}
\lstset{
  basicstyle={\footnotesize\ttfamily},
  frame=l,
  captionpos=b,
  keepspaces=true,
  keywordstyle=\bfseries,
  numbers=left,
  numbersep=5pt,
  numberstyle=\small,
  stepnumber=1,
  stringstyle=\itshape,
  tabsize=2,
  xleftmargin={0.75cm}
}

\captionsetup[table]{name=Tabla}
\newcommand\tablink[1]{\hyperref[#1]{en la Tabla~\ref*{#1}}}
\newcommand\Tablink[1]{\hyperref[#1]{La Tabla~\ref*{#1}}}
\renewcommand{\floatpagefraction}{.8}
\newcommand\figlink[2]{\hyperref[#1]{en la Figura~\ref*{#1}{#2}}}
\newcommand\Figlink[2]{\hyperref[#1]{La Figura~\ref*{#1}{#2}}}
\newcommand\Fig[2]{\hyperref[#1]{(Fig.~\ref*{#1}{#2})}}
\newcommand\Alg[1]{\hyperref[#1]{(Algoritmo~\ref*{#1})}}
\newcommand\Alglink[1]{\hyperref[#1]{El Algoritmo~\ref*{#1}}}

\makeatletter
\providecommand\phantomsection{}

\newcommand\listofillustrations{
    \chapter*{Lista de Ilustraciones}
    \phantomsection
    \addcontentsline{toc}{chapter}{Lista de Ilustraciones}
    \section*{Figuras}
    \phantomsection
    \addcontentsline{toc}{section}{\protect\numberline{}Figuras}
    \@starttoc{lof}
    \bigskip
    \section*{Tablas}
    \phantomsection
    \addcontentsline{toc}{section}{\protect\numberline{}Tablas}
    \@starttoc{lot}
    \bigskip
    \section*{Algoritmos}
    \phantomsection
    \addcontentsline{toc}{section}{\protect\numberline{}Algoritmos}
    \@starttoc{lol}}

\makeatother

\begin{document}

% Front Matter
\input{chapters/titlepage}

\chapter*{Resumen}
\input{chapters/abstract}

\tableofcontents

\listofillustrations

% Main Matter
\chapter*{Introducción}
\input{chapters/introduction}

\chapter{Técnicas de Visualización de Historiales de Wikis}
\input{chapters/chapter01}

\chapter{Visualizaciones de Datos}
\input{chapters/chapter02}

\chapter{Tecnologías para las Visualizaciones Web}
\input{chapters/chapter03}

\chapter{Marco Metodológico}
\input{chapters/chapter04}

\chapter*{Conclusiones}
\input{chapters/conclusion}

% Appendix
\begin{appendices}
  \chapter{Taxonomía de los Métodos de Visualización de Datos}
  \input{chapters/appendix}
\end{appendices}

% Back Matter
\printbibliography

\end{document}
